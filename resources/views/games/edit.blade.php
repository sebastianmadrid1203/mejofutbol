    ...
    <form action="{{ route('games.update', $game->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="date" id="date" name="date" class="form-control @error('date') is-invalid @enderror"
                value="{{ old('date', date('Y-m-d', strtotime($game->date))) }}" required>
            @error('date')
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="number" id="local_goals" name="local_goals" class="form-control @error('local_goals') is-invalid @enderror"
                    value="{{ old('local_goals', $game->local_goals) }}" min="0" required>
                @error('local_goals')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="number" id="away_goals" name="away_goals" class="form-control @error('away_goals') is-invalid @enderror"
                    value="{{ old('away_goals', $game->away_goals) }}" min="0" required>
                @error('away_goals')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="mb-3">
            <label for="teams" class="form-label">Equipos Participantes (seleccione al menos dos)</label>
            <select name="teams[]" id="teams" class="form-select @error('teams') is-invalid @enderror" multiple required>
                @foreach($teams as $team)
                    <option value="{{ $team->id }}" {{ in_array($team->id, old('teams', $game->teams->pluck('id')->toArray())) ? 'selected' : '' }}>
                        {{ $team->name }}
                    </option>
                @endforeach
            </select>
            @error('teams')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            <small class="form-text text-muted">Mantenga presionada Ctrl (Cmd en Mac) para seleccionar varios equipos.</small>
        </div>
