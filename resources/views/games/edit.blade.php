        ...
        <div class="col-md-8">
            <div class="card shadow-lg border-0 animate__animated animate__fadeInDown">
                <div class="card-header bg-gradient-header text-white d-flex align-items-center">
                    ...
                    <span class="fs-5 fw-bold">Editar Juego #{{ $game->id }}</span>
                </div>
                <div class="card-body">
                    <form action="{{ route('games.update', $game->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="date" id="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date', $game->date) }}" required>
                            @error('date')
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" name="local_goals" id="local_goals" min="0" class="form-control @error('local_goals') is-invalid @enderror" value="{{ old('local_goals', $game->local_goals) }}" required>
                                @error('local_goals')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" name="away_goals" id="away_goals" min="0" class="form-control @error('away_goals') is-invalid @enderror" value="{{ old('away_goals', $game->away_goals) }}" required>
                                @error('away_goals')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="d-flex gap-2 flex-column flex-md-row">
                                ...
                                <select name="teams[]" class="form-select @error('teams') is-invalid @enderror" required>
                                    ...
                                    @foreach($teams as $team)
                                        <option value="{{ $team->id }}" {{ $team->id == ($teamIds[0] ?? '') ? 'selected' : '' }}>
                                            {{ $team->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="teams[]" class="form-select @error('teams') is-invalid @enderror" required>
                                    ...
                                    @foreach($teams as $team)
                                        <option value="{{ $team->id }}" {{ $team->id == ($teamIds[1] ?? '') ? 'selected' : '' }}>
                                            {{ $team->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="d-flex justify-content-between mt-4">
                            <button type="submit" class="btn btn-custom">
                                <i class="bi bi-check-circle me-1"></i> Actualizar Juego
                            </button>
